...
def multi_pred(
    sess, y_pred, seq, batch_size, n_his, n_pred, step_idx, dynamic_batch=True
):
    """
    ...
    for i in gen_batch(seq, min(batch_size, len(seq)), dynamic_batch=dynamic_batch):
        ...
        for j in range(n_pred):
            pred = sess.run(
                y_pred, feed_dict={"data_input:0": test_seq, "keep_prob:0": 1.0}
            )
...
def model_inference(
    ...
    """
    x_val, x_test, x_stats = (
        inputs.get_data("val"),
        inputs.get_data("test"),
        inputs.get_stats(),
    )
    ...
    if sum(chks):
        ...
        y_pred, len_pred = multi_pred(
            sess, pred, x_test, batch_size, n_his, n_pred, step_idx
        )
        evl_pred = evaluation(
            x_test[0:len_pred, step_idx + n_his, :, :], y_pred, x_stats
        )
        min_val = evl_pred
